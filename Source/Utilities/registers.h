...
#define lpe_ba m_BAR0.Base.baseptr
#define pci_ba m_BAR1.Base.baseptr
...
#define catpt_inbox_addr(cdev) \
	((cdev)->lpe_ba + (cdev)->ipc_config.inbox_offset)
#define catpt_outbox_addr(cdev) \
	((cdev)->lpe_ba + (cdev)->ipc_config.outbox_offset)
...
#define catpt_readl_poll_shim(cdev, reg, val, mask, delay_us, timeout_us) \
	readl_poll_timeout(catpt_shim_addr(cdev) + CATPT_SHIM_##reg, \
				val, mask, delay_us, timeout_us)
...
#define catpt_readl_pci_raw(cdev, reg) \
	readl(cdev->pci_ba + reg)
#define catpt_writel_pci_raw(cdev, reg, val) \
	writel(val, cdev->pci_ba + reg)

#define catpt_updatel_pci(cdev, reg, mask, val) \
	catpt_writel_pci(cdev, reg, \
			 (catpt_readl_pci(cdev, reg) & ~(mask)) | (val))

#define catpt_updatel_pci_raw(cdev, reg, mask, val) \
	catpt_writel_pci_raw(cdev, reg, \
			 (catpt_readl_pci_raw(cdev, reg) & ~(mask)) | (val))

#define catpt_readl_poll_pci(cdev, reg, mask, cond, delay_us, timeout_us) \
	readl_poll_timeout((cdev)->pci_ba + CATPT_PCI_##reg, \
			   mask, cond, delay_us, timeout_us)
